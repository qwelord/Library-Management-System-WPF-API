...
public partial class BooksView : UserControl
{
    ...
    private async void DeleteButton_Click(object sender, RoutedEventArgs e)
    {
        ...
        if (MessageBox.Show($"Удалить книгу '{ViewModel.SelectedBook.Title}'?", "Подтверждение", MessageBoxButton.YesNo) == MessageBoxResult.Yes)
        {
            if (ViewModel.ApiClient == null) return;
            try
            {
                await ViewModel.ApiClient.DeleteAsync($"books/{ViewModel.SelectedBook.Id}");
                ViewModel.LoadBooksCommand?.Execute(ViewModel.SearchText);
            }
            catch (Exception ex)
            {
